...
class Bar
{
	...
	public function renderLoader(DeferredContent $defer): void
	{
		...
		$async = true;
		require \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.10.x/src/Tracy/Bar' . '/assets/loader.phtml';
	}
	...
	private function renderPartial(string $type, string $suffix = ''): array
	{
		...
		return [
			'bar' => Helpers::capture(function () use ($type, $panels) {
				require \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.10.x/src/Tracy/Bar' . '/assets/bar.phtml';
			}),
			'panels' => Helpers::capture(function () use ($type, $panels) {
				require \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.10.x/src/Tracy/Bar' . '/assets/panels.phtml';
			}),
		];
	}
