...
class BlueScreen
{
	...
	{
		$this->collapsePaths = preg_match('#(.+/vendor)/tracy/tracy/src/Tracy/BlueScreen$#', strtr(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.10.x/src/Tracy/BlueScreen', '\\', '/'), $m)
			? [$m[1] . '/tracy', $m[1] . '/nette', $m[1] . '/latte']
			: [dirname(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.10.x/src/Tracy/BlueScreen')];
	...
	public function render(\Throwable $exception): void
	{
		...
		$this->renderTemplate($exception, \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.10.x/src/Tracy/BlueScreen' . '/assets/page.phtml');
	}
	...
	{
		$defer->addSetup('Tracy.BlueScreen.loadAjax', Helpers::capture(fn() => $this->renderTemplate($exception, \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.10.x/src/Tracy/BlueScreen' . '/assets/content.phtml')));
	}
	...
	public function renderToFile(\Throwable $exception, string $file): bool
	{
		if ($handle = @fopen($file, 'x')) {
			...
			$this->renderTemplate($exception, \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.10.x/src/Tracy/BlueScreen' . '/assets/page.phtml', false);
			ob_end_flush();
			ob_end_clean();
			fclose($handle);
			return true;
	...
	private function renderTemplate(\Throwable $exception, string $template, bool $toScreen = true): void
	{
		...
		$css = array_map('file_get_contents', array_merge([
			\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.10.x/src/Tracy/BlueScreen' . '/../assets/reset.css',
			\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.10.x/src/Tracy/BlueScreen' . '/assets/bluescreen.css',
			\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.10.x/src/Tracy/BlueScreen' . '/../assets/toggle.css',
			\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.10.x/src/Tracy/BlueScreen' . '/../assets/table-sort.css',
			\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.10.x/src/Tracy/BlueScreen' . '/../assets/tabs.css',
			\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.10.x/src/Tracy/BlueScreen' . '/../Dumper/assets/dumper-light.css',
